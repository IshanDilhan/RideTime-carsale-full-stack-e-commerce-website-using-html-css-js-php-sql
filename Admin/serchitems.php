...
<style>
  ...
  table {
    width: 100%; /* Ensure the table takes up all available width */
    max-width: 1500px; /* Remove any max-width constraint */
    margin: 0; /* Remove any top, right, and bottom margins */
    margin-left: -50px; /* Add a left margin of 50px */
    // margin-right: -50px; /* This line is commented out */
  ...
  th, td {
  max-width: 50px;
  /* Let th and td elements expand as needed */
  margin: 0; /* Remove any margins */
  padding: 0; /* Remove any padding */
  /* You can define other styles for th and td on larger screens here */
}
...
function enableDesable1(itemID,enableOrDesable){
    $.ajax({
         type: "POST",
         url: "ajax/enable.php",
         data: "itemId=" + itemID + "&enable=" + enableOrDesable,
...
<?php
$searchQ = "SELECT `car_id`, `car_name`, `car_brand`, `car_status`, `display_price`, `seling_price`, `engine_power`, `km`, `giar_type`, `car_image_url`, `car_is_enable` FROM `cars` ORDER BY `car_id` DESC";
...
?>
    <td>
        <img style="height: 25px; width: 25px;cursor: pointer;" src="./../assets/images/correct.png" alt="enable" onclick="enableDesable1(<?php echo $row['car_id']; ?>, 'd')">
    </td>
<?php
} else {
?>
    <td>
        <img style="height: 25px; width: 25px;cursor: pointer;" src="./../assets/images/wrong.png" alt="disable" onclick="enableDesable1(<?php echo $row['car_id']; ?>, 'e')">
    </td>
<?php
}
?>
