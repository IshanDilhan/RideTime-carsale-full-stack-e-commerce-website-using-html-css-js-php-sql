...
if ($_SERVER['REQUEST_METHOD'] === 'POST' ) {
	...
	if(isset($_POST['username']) && isset($_POST['pass'])){
        ...
        FROM`car_user`
        WHERE `user_name`='$username' and `pwd`='$pass' and `user_cato`= 3
		AND `user_is_active`=1 
		LIMIT 1" ;
        
        $selectUser1 = "SELECT `user_id`,`user_cato`,`user_name`,`pwd`,`email`,`phone_no`,
        `user_image_url`,`user_is_active`,`last_login`,`created_date`
        FROM`car_user`
        WHERE `user_name`='$username' and `pwd`='$pass' and `user_cato`= 1
		AND `user_is_active`=1 
		LIMIT 1" ;
        ...
        $result1 = mysqli_query($con,$selectUser1);
